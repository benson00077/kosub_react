...
function Sidebar({ isLoggedIn }) {
  ...
  const linkclass = `flex flex-row justify-between my-2 pt-3 pb-3 pr-2 pl-2 rounded hover:bg-slate-500 opacity-80`
  const activeNavclass = (path) => (location.pathname === path ? ' bg-slate-600' : '')

  return (
    <>
      <Link to={NAV.HOME.path}>
        <IconHome class={`w-18 md:w-24 fill-slate-50 ${linkclass + activeNavclass(NAV.HOME.path)}`} />
      </Link>

      <div class="mt-5">
        <div class="mb-4 text-zinc-400">Pages</div>
        <div>
          <Link to={NAV.ABOUT.path} class={linkclass + activeNavclass(NAV.ABOUT.path)}>
          ...
          <Link to={NAV.GUIDE.path} class={linkclass + activeNavclass(NAV.GUIDE.path)}>
            <svg
          ...
          <Link to={NAV.SEARCH.path} class={linkclass + activeNavclass(NAV.SEARCH.path)}>
            <svg class="fill-slate-50 w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
              <path d="M12.9 14.32a8 8 0 111.41-1.41l5.35 5.33-1.42 1.42-5.33-5.34zM8 14A6 6 0 108 2a6 6 0 000 12z" />
            </svg>
            <h4 class="text-gray-300">{NAV.SEARCH.title}</h4>
          </Link>
        </div>
      </div>

      <div class="mt-5">
        <div class="mb-4 text-zinc-400">Account</div>
        <div>
          {isLoggedIn && (
            <Link to={NAV.FAVORITE.path} class={linkclass + activeNavclass(NAV.FAVORITE.path)}>
              <h4 class="text-gray-300">{NAV.FAVORITE.title}</h4>
            </Link>
          )}

          {!isLoggedIn && (
            <Link to={NAV.LOGIN.path} class={linkclass + activeNavclass(NAV.LOGIN.path)}>
              <svg class="fill-slate-50 w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                <path d="M5 5a5 5 0 0110 0v2A5 5 0 015 7V5zM0 16.68A19.9 19.9 0 0110 14c3.64 0 7.06.97 10 2.68V20H0v-3.32z" />
              </svg>
              <h4 class="text-gray-300">{NAV.LOGIN.title}</h4>
            </Link>
          )}

          {!isLoggedIn && (
            <Link to={NAV.REGISTER.path} class={linkclass + activeNavclass(NAV.REGISTER.path)}>
              ...
              <h4 class="text-gray-300">{NAV.REGISTER.title}</h4>
            </Link>
          )}
        </div>
      </div>
      ...
      <div class={`mt-5 transition-opacity ${showToolkit ? 'opacity-100' : '-translate-x-52 opacity-0'}`}>
        <div class="mb-4 text-zinc-400">Interaction</div>
        <ApiButtonsToolKit withRemoveFavorite={location.pathname === NAV.FAVORITE.path} />
      </div>

      <div class={`mt-5 transition-opacity ${showToolkit ? 'opacity-100' : '-translate-x-52 opacity-0'}`}>
        <div class="mb-4 text-zinc-400">Preference</div>
        <SentencesToolKit />
      </div>
    </>
  )
